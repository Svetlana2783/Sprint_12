...
class TestYaScooterHomePage:
    ...
    def test_click_top_order_button_show_order_page(self, setup_method):
        ya_scooter_home_page = setup_method
        ya_scooter_home_page.click_top_order_button()
        ya_scooter_home_page.verify_url(Urls.ORDER_PAGE)
    ...
    @allure.description('Проверка что, на домашней странице в блоке "Как это работает" по кнопке "Заказать", '
                        'просходит корректный переход на страницу "Оформления заказа".')
    def test_click_bottom_order_button_show_order_page(self, setup_method):
        ya_scooter_home_page = setup_method
    ...
    def test_click_yandex_button_go_to_yandex(self, setup_method):
        ya_scooter_home_page = setup_method
